...
//
// V20 appended a 292-byte auxiliary output table; V21 drops it again, because
...
// Identical to the live V21 layout.  The embedded types are still the live ones
// only because nothing has grown them since V19; the size assert below pins
// that, and growing any of them means snapshotting a _v19 variant here first.
typedef struct __attribute__((packed)) {
    uint32_t magic;
    uint16_t version;                        // == 19
    uint16_t reserved;
...
// --- Preset Directory v20 (kept only for upgrade migration) ---
// The V19 layout plus the auxiliary output table V21 dropped.  The block is
// opaque here because the CsAuxConfig type is gone; the migration copies the
// prefix ahead of it and discards it.
typedef struct __attribute__((packed)) {
    uint32_t magic;
    uint16_t version;                        // == 20
    uint16_t reserved;
    ...
    CsMacroConfig cs_macros;                 // 1060 bytes
    CsDisplayFlash cs_display;               // 80 bytes
    uint8_t cs_aux[292];                     // dropped by V21
} PresetDirectory_v20;
...
// V20 only differs by the trailing aux table, so the V20->V21 migration is a
// prefix copy that stops at cs_aux; everything ahead of it must match V21.
_Static_assert(offsetof(PresetDirectory_v20, cs_display) == offsetof(PresetDirectory, cs_display),
               "V20 and V21 directories must share a byte-identical pre-cs_aux prefix");
// Pins the on-flash V20 geometry, which the offset check above cannot.
...
static bool dir_load_cache(void) {
    ...
    if (flash_dir->version == 20) {
        ...
        const uint8_t *v20_data_start = (const uint8_t *)&v20->startup_mode;
        size_t v20_data_len = sizeof(PresetDirectory_v20) - offsetof(PresetDirectory_v20, startup_mode);
        if (crc32(v20_data_start, v20_data_len) != v20->crc32) {
            dir_cache_valid = false;
            return false;
        }
        memset(&dir_cache, 0, sizeof(dir_cache));
        // Header excluded: dir_flush() restamps magic/version/crc, and copying
        // the old one would leave the cache reading V20 until it does.
        memcpy(&dir_cache.startup_mode, v20_data_start,
               offsetof(PresetDirectory_v20, cs_aux) - offsetof(PresetDirectory_v20, startup_mode));
        dir_sanitize_ctrl_iface();
        dir_sanitize_cs_config();
        dir_sanitize_cs_ir();
        dir_sanitize_cs_groups();
...
static void cs_config_from_v1(CsFlashConfig *dst, const CsFlashConfig_v1 *src) {
    ...
    for (int s = 0; s < 8; s++) {
        ...
        // event / target / index / reserved / delays / extras / reserved2 stay zero.
    }
